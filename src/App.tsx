...
function App() {
  const { user } = useUser()
  const [signInDialogState, setSignInDialogState] = React.useState<boolean>(false);
      ...
      <div className='flex flex-col-reverse h-screen w-screen dark:bg-black'>
        ...
        <div className='flex flex-row-reverse'>
          <div className='flex flex-row justify-center m-2 space-x-4'>
            <ModeToggle />
            <SiginInModal setSignInDialogState={setSignInDialogState} signInDialogState={signInDialogState} />
            {
              user ?
                <CustomAvatar url='RS' /> :
                <Button type='button' onClick={() => setSignInDialogState(true)}>Sign In</Button>
            }
          </div>
        </div>
      </div>
